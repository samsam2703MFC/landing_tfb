...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const favicon = storageUrl(assets.favicon);
...
/**
 * Storage paths turned into URLs the browser can actually fetch. themeAssets keeps
 * them as /storage/… because that is what it validates; storageUrl knows both the
 * /api/storage route and the basePath, and an installation served under a sub-path
 * would 404 on every logo without it.
 */
function servedAssets(values: ConfigValues) {
  const assets = themeAssets(values);
  return {
    logo: storageUrl(assets.logo),
    logoInverse: storageUrl(assets.logoInverse),
    favicon: storageUrl(assets.favicon),
    fontsHref: assets.fontsHref,
  };
}
